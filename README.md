...
Open `http://localhost:3000`. Without a Cloudflare country header, the root URL redirects to `/en`. To check country routing locally, send a `CF-IPCountry: VN`, `TH`, or `KR` request header. `pnpm build` checks the production build and TypeScript.
...
Each page has four indexable URLs under `/vi`, `/th`, `/en`, and `/ko`, with translated titles, descriptions, Open Graph and Twitter metadata, canonical links, and reciprocal `hreflang` links. `/sitemap.xml` lists the 24 localized pages; `/robots.txt` points to it. Set `SITE_URL` at build time only if the public origin differs from `https://justiceforpubgvn.com`.

Unprefixed page URLs redirect according to the visitor's IP country: Vietnam → Vietnamese, Thailand → Thai, South Korea → Korean, and other or unknown countries → English. An explicit localized URL always wins. The language dropdown sets a one-year preference cookie and keeps the current page, query string, and fragment when switching languages; the cookie takes precedence on future unprefixed visits. Browser language settings are not used.
...
- `/{lang}/players` links to individual four-language profiles for Himass and TanVuu. Their locally stored portraits come from PUBG Esports player pages; each profile links to the original portrait, career sources, and a player YouTube channel listed in PUBG's PNC 2026 POV directory.
