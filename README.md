...
A Next.js timeline of the PUBG Asia Stars case in 10 languages: Vietnamese, Thai, English, Korean, Simplified Chinese, Japanese, Russian, Indonesian, Spanish, and Filipino. It follows the incident and official responses from September 17 through September 23, 2026. The original single-file page is retained in `legacy/index.html`.
...
Open `http://localhost:3000`. Without a Cloudflare country header, the root URL redirects to `/en`. To check country routing locally, send a `CF-IPCountry: VN`, `TH`, `KR`, `CN`, `JP`, `RU`, `ID`, `ES`, or `PH` request header. `pnpm build` checks the production build and TypeScript.
...
Each page has indexable URLs under `/vi`, `/th`, `/en`, `/ko`, `/zh`, `/ja`, `/ru`, `/id`, `/es`, and `/tl`, with translated titles, descriptions, Open Graph and Twitter metadata, canonical links, and reciprocal `hreflang` links. `/sitemap.xml` lists the 60 localized pages; `/robots.txt` points to it. Set `SITE_URL` at build time only if the public origin differs from `https://justiceforpubgvn.com`.

Unprefixed page URLs redirect according to the visitor's IP country: Vietnam → Vietnamese, Thailand → Thai, South Korea → Korean, China → Simplified Chinese, Japan → Japanese, Russia → Russian, Indonesia → Indonesian, Spain → Spanish, Philippines → Filipino, and other or unknown countries → English. An explicit localized URL always wins. The language dropdown sets a one-year preference cookie and keeps the current page, query string, and fragment when switching languages; the cookie takes precedence on future unprefixed visits. Browser language settings are not used.
...
- `data/timeline.ts` contains the dated events, what happened, attributed responses, explanations, and source IDs in Vietnamese and English. Thai and Korean UI copy is in `data/ui-translations.ts`; other language copies are in `data/zh-translations.ts`, `data/ja-translations.ts`, `data/ru-translations.ts`, `data/id-translations.ts`, `data/es-translations.ts`, and `data/tl-translations.ts`.
...
- `/{lang}/players` links to individual localized profiles for Himass and TanVuu. Their locally stored portraits come from PUBG Esports player pages; each profile links to the original portrait, career sources, and a player YouTube channel listed in PUBG's PNC 2026 POV directory.
